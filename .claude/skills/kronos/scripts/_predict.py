...
def main() -> int:
    if len(sys.argv) < 5:
        print("Usage: _predict.py <TICKER> <period> <interval> <pred_len>", file=sys.stderr)
        return 1
    ...
    history = fetch_ohlcv(ticker, period, interval)
    ...
    json_out = os.environ.get("KRONOS_JSON_OUT")
    if json_out:
        import json
        ...
        def rows(df: pd.DataFrame) -> list:
            ...
            for ts, row in df.iterrows():
                out.append({"t": pd.Timestamp(ts).strftime("%Y-%m-%d %H:%M"), **{k: round(float(v), 4) for k, v in row.items()}})
            return out
        ...
        print(f"[kronos] wrote forecast JSON to {json_out}", file=sys.stderr)
